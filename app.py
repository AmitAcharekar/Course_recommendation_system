...
def recommend_course(df, title, numrec=12):
    # Assuming that cosine_sim_mat is a global variable or available in the scope
    
    # Ensure 'course_title' is present in the DataFrame
    if 'course_title' not in df.columns:
        raise ValueError("DataFrame must contain 'course_title' column.")
    
    # Convert titles to lowercase
    df['course_title_lower'] = df['course_title'].str.lower()
    title_lower = title.lower()

    # Create or update the course_index
    course_index = pd.Series(df.index, index=df['course_title_lower']).drop_duplicates()

    # Handle the case where the title is not in the course_index
...
def searchterm(term, df):
    ...
    result_df = df[df['course_title'].str.lower().str.contains(term.lower())]
    top6 = result_df.sort_values(by='num_subscribers', ascending=False).head(12)
    return top6
